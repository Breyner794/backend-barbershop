...
export const addTimeSlot = async (req, res) => {
  try {
    ...
    if (!availability) {
      availability = new Availabilitybarber({
        barberId,
        dayOfWeek: day,
        timeSlots: [],
        isWorkingDay: true,
    ...
    // Verificar solapamiento interno (validación existente)
    const convertTimeToMinutes = (timeString) => {
      const [hours, minutes] = timeString.split(":").map(Number);
      return hours * 60 + minutes;
    };
    ...
    // Crear los timeSlots temporales para validación
    const newTimeSlots = [...availability.timeSlots, { startTime, endTime }];
